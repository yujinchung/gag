...
export function visibleRoles(myRoles, phase) {
  if (phase === 'probe') return myRoles.filter((r) => r !== 'restorer');
...
export function settleCause(st) {
  ...
  if (!st.restore?.guess) return '복원안이 전송되지 않았습니다.';
  if (worst && worst.clean < 50 && bodyLow) {
    return `청정 ${worst.clean} 질의에서 오염이 들어갔고, 복원 본체가 갈렸습니다.`;
  }
  if (worst && worst.clean < 60) {
    return `가장 낮은 질의 청정 ${worst.clean}. ${worst.note || ''}`.trim();
  }
  return scores.note || '대조가 끝났습니다.';
}
