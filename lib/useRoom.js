...
export function useRoom({ code, name, onAction }) {
  ...
  useEffect(() => {
    ...
    meRef.current = me;
    const { client: rt, mode } = getRealtime();
    if (!rt) return;
    ...
    ch.on('presence', { event: 'sync' }, () => {
      ...
      setMembers(list);
      isEngineRef.current = list.length > 0 && list[0].id === me.id;
      setIsEngine(isEngineRef.current);
      if (list.length !== memberCountRef.current) {
        memberCountRef.current = list.length;
        replyStateIfEngine();
      }
    });
